...

{
  ...
  security.sudo.extraRules = [
    # Allow execution of "upsmon-notify" by user `nutmon` and group `nutmon`
    # without requiring a password.
    {
      users = [ "${config.power.ups.upsmon.user}" ];
      groups = [ "${config.power.ups.upsmon.group}" ];
      commands = [
        { command = "${pkgs.upsmon-notify}"; options = [ "NOPASSWD" ]; }
      ];
    }
  ];
  ...
  power.ups = {
    ...
    upsmon = {
      ...
      settings = {
        ...
        NOTIFYCMD = "${pkgs.writeShellScript "upsmon-notify-wrapper" ''
          sudo ${pkgs.upsmon-notify} "$@"
        ''}";
