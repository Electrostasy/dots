...
export PATH="@notify-send-all@/bin:$PATH"

function notify-send {
	command notify-send-all -a 'mdadm' -i 'drive-harddisk' -c 'device' "$@"
}
...
case "$event" in
	...
	notify-send -u 'low' \
        ...
        ;;
    *)
        echo "Unhandled message for event $event, array $array, device $device." | systemd-cat -p err
	;;
esac
