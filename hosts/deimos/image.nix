...

{
  ...
  image.repart.partitions."10-esp".contents = {
    ...
    "/config.txt".source = pkgs.writeText "config.txt" ''
      ...
      # U-Boot will also not continue the boot process unless this is present,
    ...
    # The bootloader, loaded by the SoC on boot. It performs some very basic
    # setup, and then loads one of the start*.elf files.
