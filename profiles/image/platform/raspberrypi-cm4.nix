...

{
  image.repart.partitions."10-esp".contents = {
    # Kernel image file that corresponds to the Raspberry Pi Compute Module 4
    ...
    "/config.txt".source = pkgs.writeText "config.txt" ''
      ...
      # If we try to access the dwc or XHCI when the firmware hasn't
      # initialized it, the system will freeze. This signals to the firmware to
