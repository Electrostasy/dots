...
-- TODO: Configurable preview via preview_item or similar to support code actions etc.
...
-- TODO: Don't decorate empty list window.
...
local state = {
  ...
  items = {},
  matches = {},
  format_item = function(item) return tostring(item.value) end,
...
local select_item = function(row)
  ...
  vim.api.nvim_exec_autocmds('CursorMoved', { buffer = state.list_bufnr })
end
...
local update_list = function()
  ...
  vim.api.nvim_buf_clear_namespace(state.list_bufnr, ns_selection, 0, -1)

  local lines
  local input = vim.api.nvim_buf_get_lines(state.prompt_bufnr, 0, 1, false)[1]
  if #input > 0 then
    state.matches = vim.fn.matchfuzzypos(state.items, input, { text_cb = state.format_item })
    lines = vim.iter(state.matches[1]):map(state.format_item):totable()
  else
    table.clear(state.matches)
    lines = vim.iter(state.items):map(state.format_item):totable()
  end

  vim.api.nvim_buf_set_lines(state.list_bufnr, 0, -1, true, lines)
  select_item(1)
  vim.api.nvim_set_option_value('cursorline', #state.matches ~= 0 or #state.items ~= 0, { win = state.list_winid })
  vim.api.nvim_exec_autocmds('CursorMoved', { buffer = state.list_bufnr })
end

local resize = function()
  if
    not vim.api.nvim_win_is_valid(state.prompt_win) or
  ...
  local preview_hidden = preview_width < preview_hidden_threshold
...
local line_number_decorator = function(_winid, bufnr, row)
  ...
  vim.api.nvim_buf_set_extmark(bufnr, ns_selection, row, -1, {
    id = id,
...
local open_picker = function(items, opts, on_choice)
  ...
  if opts.format_item == nil then
    state.format_item = function(item)
  ...
  local decorators = {}
  if opts.kind:find('file') or opts.kind:find('buffer') then
    table.insert(decorators, devicon_decorator)
  end

  if opts.kind:find('buffer') then
  ...
  -- Cache buffers that are previewed so as to not re-read whole files each time we select their item in the list.
  -- They are deleted when the picker is closed.
  state.previewed_buffers = {}

  state.prompt_win = vim.api.nvim_open_win(state.prompt_bufnr, true, {
    relative = 'editor',
    row = 0,
    col = 0,
    width = 1,
  ...
  vim.api.nvim_set_option_value('winhighlight', 'NormalFloat:Normal', { win = state.prompt_win })
  ...
  vim.api.nvim_set_option_value('number', true, { win = state.list_winid })
  vim.api.nvim_set_option_value('statuscolumn', '%l %s', { win = state.list_winid })
...
local update_preview = function()
  ...
  local item = vim.api.nvim_buf_get_lines(state.list_bufnr, row - 1, row, true)[1]

  local bufnr
  if item ~= nil and vim.fn.filereadable(item) == 1 and vim.system({ 'file', '--brief', '--mime-encoding', item }):wait().stdout ~= 'binary\n' then
  ...
  local lang = vim.filetype.match({ filename = item })
...
vim.keymap.set('n', '<leader>e', function()
  ...
  vim.ui.select(items, { prompt = 'Files', kind = 'files' }, vim.cmd.edit)
end)

vim.keymap.set('n', '<leader>b', function()
  local items = vim.iter(vim.api.nvim_list_bufs()):filter(function(buf) return vim.fn.buflisted(buf) == 1 end):totable()
  vim.ui.select(items, { prompt = 'Buffers', kind = 'buffers', format_item = vim.fn.bufname, }, vim.api.nvim_set_current_buf)
end)
